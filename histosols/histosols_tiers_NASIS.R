...
r02_histosols$lieutex <- as.character(r02_histosols$lieutex)
...
site(r02_histosols) <- hcs.bounds

r02_histosols_hascolor <- filter(mutate_profile(r02_histosols, 
                                                hascolor = all(!is.na(soil_color)), 
                                                hasdesgn = all(!is.na(hzname))),
                                 hascolor, hasdesgn)

set1 <- filter(r02_histosols_hascolor, hcs_b == 130)[1:10]
site(set1)$grp <- "non-fibric (85% of R02 NASIS Histosols)"

set2 <- filter(r02_histosols_hascolor, hcs_b == 160)[1:10]
site(set2)$grp <- "fibric (15% of R02 NASIS Histosols)"

par(mar=c(0,0,0,0))
groupedProfilePlot(union(list(set1, set2)), label='upedonid', groups="grp",
     name.style='left-center', cex.names=0.75, max.depth=200)


## now look at some mlra 15 histosols
f <- fetchKSSL(bbox = c(-122.66, 38.36, -121.58, 37.75))
f.hist <- filter(f, tax)
